  ...
  useEffect(() => {
    async function fetchData(params: any) {
      ...
      setTimeout(() => {
        ...
        const convertedPrice = prodottiData.prezzo?.replace(",", ".");
        const convertedDiscount = prodottiData.sconto?.replace(",", ".");

        if (
          prodottiData.sconto !== undefined ||
          prodottiData.percentuale !== undefined
        ) {
          if (prodottiData.sconto !== undefined) {
            const scontoValue = prodottiData.sconto?.replace(",", ".");
            setSconto(Number(scontoValue));
          } else {
            const prezzoValue = Number(prodottiData.prezzo?.replace(",", "."));
            const percentualeValue = Number(
              prodottiData.percentuale?.replace(",", ".")
            );
            const scontoValue = (
              prezzoValue -
          ...
          prodottiData.percentuale !== undefined
            ? setPercentuale(prodottiData.percentuale)
            : setPercentuale(
                Math.round(
